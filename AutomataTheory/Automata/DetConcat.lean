...
-/
theorem da_concat_det_run_1 (as : Stream' A) (k : ℕ) :
...
theorem da_concat_det_run_2 (as : Stream' A) (n : ℕ) (i : Fin (Nat.card M2.State + 2))
    ...
    ∃ m < n, M1.DetRun as m ∈ acc1 ∧
      ∀ k > m, ((M1.Concat acc1 M2).DetRun as k).2 i = some (M2.DetRun (as.drop m) (k - m)) := by
...
theorem da_concat_ptr2_exists (as : Stream' A) (n : ℕ) (h_n : M1.DetRun as n ∈ acc1) (k : ℕ) :
  ...
  let I := {j | ((M1.Concat acc1 M2).DetRun as (n + k + 1)).2 j = some (M2.DetRun (as.drop n) (k + 1))}
  have h_fin : I.Finite := by exact toFinite I
  ...
  have h_min' : ∀ j' < j, ¬((M1.Concat acc1 M2).DetRun as (n + k + 1)).2 j' = some (M2.DetRun (as.drop n) (k + 1)) := by
    intro j' h_j' h_contra ; have := h_min h_contra ; fin_omega
  constructorm* _ ∧ _
  · grind
...
-/
noncomputable def DA.ConcatPtr2 (as : Stream' A) (n : ℕ) (h_n : M1.DetRun as n ∈ acc1) (k : ℕ) : Fin (Nat.card M2.State + 2) :=
...
-/
theorem da_concat_ptr2_det_run (as : Stream' A) (n : ℕ) (h_n : M1.DetRun as n ∈ acc1) (k : ℕ) :
    ((M1.Concat acc1 M2).DetRun as (n + k + 1)).2 (DA.ConcatPtr2 M1 acc1 M2 as n h_n k) =
    some (M2.DetRun (as.drop n) (k + 1)) := by
...
-/
theorem da_concat_ptr2_eventually (as : Stream' A) (n : ℕ) (h_n : M1.DetRun as n ∈ acc1):
...
theorem da_concat_to_muller_accept (as : Stream' A) (n : ℕ)
  ...
  have h_suffix : ∀ k ≥ m, ((M1.Concat acc1 M2).DetRun as k).2 i = some (M2.DetRun (as.drop n) (k - n)) := by
    intro k h_k
    ...
    exact h_p
  use i ; constructor
  · suffices h_inf : {s2 | ∃ s ∈ InfOcc ((M1.Concat acc1 M2).DetRun as), s.2 i = some s2} = InfOcc (M2.DetRun (as.drop n)) by
      simpa [h_inf]
    ext s2 ; constructor
    · rintro ⟨s, h_inf, h_s⟩
...
theorem da_concat_of_muller_accept (as : Stream' A)
    ...
    ∃ n, M1.toNA.FinAccept acc1 n as ∧ M2.MullerAccept accSet2 (as.drop n) := by
  ...
  suffices h_inf : {s2 | ∃ s ∈ InfOcc ((M1.Concat acc1 M2).DetRun as), s.2 i = some s2} = InfOcc (M2.DetRun (as.drop m)) by
    use m ; constructor
