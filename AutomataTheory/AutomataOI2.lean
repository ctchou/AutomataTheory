...
private lemma automata_oi2_lemma1 {as : ℕ → A} {ss : ℕ → (AutomataOI2 M acc).State}
    (h_run : InfRun (AutomataOI2 M acc) as ss) :
  ...
  · suffices h_lt : LeadsTo ss {s | s.1 0 ∈ acc 0 ∧ s.2 = 0} {s | s.1 1 ∈ acc 1 ∧ s.2 = 1} by
    ...
    have h_lt1 : LeadsTo ss {s | s.1 0 ∈ acc 0 ∧ s.2 = 0} {s | s.2 = 1} := by
      ...
      simp [AutomataOI2_HistNext, h_acc, h_hist] at h_step
      assumption
...
private lemma automata_oi2_lemma2 {as : ℕ → A} {ss : ℕ → (AutomataOI2 M acc).State}
    (h_run : InfRun (AutomataOI2 M acc) as ss)
  ...
  · have h_inter : {s : (AutomataOI2 M acc).State | s.1 0 ∈ acc 0 ∧ s.2 = 0} = {s | s.2 = 0} ∩ {s | s.1 0 ∈ acc 0} := by
      ext s ; simp ; tauto
    rw [h_inter]
    ...
    simp [AutomataOI2_HistNext, h_acc, h_hist] at h_step
    assumption
  · have h_inter : {s : (AutomataOI2 M acc).State | s.1 1 ∈ acc 1 ∧ s.2 = 1} = {s | s.2 = 1} ∩ {s | s.1 1 ∈ acc 1} := by
      ext s ; simp ; tauto
    have h_compl : {s : (AutomataOI2 M acc).State | s.2 = 0}ᶜ = {s | s.2 = 1} := by
      ext s ; simp
      constructor
...
theorem accepted_omega_lang_inter2 :
    AcceptedOmegaLang (AutomataOI2 M acc) (AutomataOI2_Acc M acc) = ⋂ i : Fin 2, AcceptedOmegaLang (M i) (acc i) := by
