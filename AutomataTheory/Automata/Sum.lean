...
theorem na_sum_fin_run {n : ℕ} {as : Stream' A} {ss : Stream' (NA.Sum M).State} :
  ...
  · rintro ⟨i, ss_i, h_run, h_ss⟩
    constructor
    · simp [NA.Sum, h_ss 0 (by omega : 0 < n + 1)]
      use i, (ss_i 0)
      simp ; exact h_run.1
    · intro k h_k
...
/-- The language accepted by the sum NA is the union of the languages
accepted by the component automata.
-/
theorem acc_lang_union :
    (NA.Sum M).AcceptedLang (NA.Sum_Acc M acc) = ⋃ i : I, (M i).AcceptedLang (acc i) := by
  ext al ; simp [NA.Sum_Acc, NA.AcceptedLang, NA.FinAccept]
  constructor
  · rintro ⟨n, as, ⟨ss, h_run, h_acc⟩, h_al⟩
    obtain ⟨i, ss_i, h_run_i, h_ss_i⟩ := na_sum_fin_run.mp h_run
    use i, n, as
    constructor
    · use ss_i
      constructor
...
/-- The ω-language accepted by the sum NA is the union of the ω-languages
accepted by the component automata.
-/
theorem acc_omega_lang_union :
    (NA.Sum M).AcceptedOmegaLang (NA.Sum_Acc M acc) = ⋃ i : I, (M i).AcceptedOmegaLang (acc i) := by
  ext as ; simp [NA.Sum_Acc, NA.AcceptedOmegaLang, NA.BuchiAccept]
  constructor
  · rintro ⟨ss, h_run, h_inf⟩
    obtain ⟨i, ss_i, h_run_i, h_ss_i⟩ := na_sum_inf_run.mp h_run
    use i, ss_i
    constructor
    · assumption
  ...
  · rintro ⟨i, ss_i, h_run_i, h_inf_i⟩
    use ((Sigma.mk i) ∘ ss_i)
    constructor
    · apply na_sum_inf_run.mpr
      use i, ss_i
    · let p k := ss_i k ∈ acc i
      let q k := ∃ i', ∃ si' ∈ acc i', ⟨i', si'⟩ = ((Sigma.mk i ∘ ss_i) k : (NA.Sum M).State)
      have h_p : ∃ᶠ k in atTop, p k := by assumption
      have h_pq : ∀ k, p k → q k := by
        simp [p, q] ; intro k h
        use i, (ss_i k)
